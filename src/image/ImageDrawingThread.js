...
import ImageDrawingDialog from '../image/ImageDrawingDialog';
import {
    STATES,
    DRAW_STATES,
...
import { getBrowserCoordinatesFromLocation, getContext } from './imageUtil';
import { createLocation, getScale } from '../util';

class ImageDrawingThread extends DrawingThread {
     ...
     * @return {ImageDrawingThread} Drawing annotation thread instance
     */
    constructor(data) {
        super(data);
    ...
    handleMove(location) {
        ...
        const [x, y] = getBrowserCoordinatesFromLocation(location, this.annotatedElement);
    ...
    /**
     * Display the imageument drawing thread. Will set the drawing context if the scale has changed since the last show.
     *
     * @public
     * @return {void}
     */
    ...
    /**
     * Creates the imageument drawing annotation dialog for the thread.
     *
     * @protected
     * @override
     * @return {void}
    ...
    createDialog() {
        ...
        this.dialog = new ImageDrawingDialog({
    ...
    checkAndHandleScaleUpdate() {
        ...
        this.imageEl = this.annotatedElement.querySelector(`[data-page-number="${this.location.page}"]`);
        this.drawingContext = getContext(this.imageEl, CLASS_ANNOTATION_LAYER_DRAW_IN_PROGRESS);
    ...
    /**
     * Requires a ImageDrawingThread to have been started with ImageDrawingThread.start(). Reconstructs a browserCoordinate
     * relative to the dimensions of the ImageDrawingThread page element.
     *
     * @private
     * @param {Location} imageumentLocation - The location coordinate relative to the imageument
     * @return {Location} The location coordinate relative to the browser
     */
    reconstructBrowserCoordFromLocation(imageumentLocation) {
        const reconstructedLocation = createLocation(
            imageumentLocation.x,
            imageumentLocation.y,
            this.location.dimensions
        );
        const [xNew, yNew] = getBrowserCoordinatesFromLocation(reconstructedLocation, this.imageEl);
    ...
    selectContext() {
        ...
        this.concreteContext = getContext(this.imageEl, CLASS_ANNOTATION_LAYER_DRAW);
    ...
    getBrowserRectangularBoundary() {
        if (!this.location || !this.location.dimensions || !this.annotatedElement) {
            return null;
        }
        ...
        const [x1, y1] = getBrowserCoordinatesFromLocation(l1, this.annotatedElement);
        const [x2, y2] = getBrowserCoordinatesFromLocation(l2, this.annotatedElement);
        const width = x2 - x1;
        const height = y2 - y1;
...
export default ImageDrawingThread;
